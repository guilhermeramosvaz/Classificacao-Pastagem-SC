...
O resultado foi recortado espacialmente para os limites do estado de Santa Catarina. A integração foi realizada através de uma operação aritmética de "transição", onde os valores das classes do MapBiomas foram multiplicados por 100 e somados aos valores das classes do GPW. A fórmula utilizada foi:

```
Valor_Integrado = (Classe_MapBiomas x 100) + Classe_GPW
```

Esta abordagem gera códigos únicos que representam cada combinação possível entre as duas fontes de dados gerando no total de 12 combinações (ex: o código 1501 representaria a Classe 15 'Pastagem' do MapBiomas integrada à Classe 1 'Cultivated grassland' do Global Pasture Watch).
